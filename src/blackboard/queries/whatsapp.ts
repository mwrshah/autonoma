...
export function getLatestPendingAction(db: SqlDatabase, channel = "whatsapp"): PendingActionRow | undefined {
...
export function getPendingActionByContextRef(db: SqlDatabase, contextRef: string): PendingActionRow | undefined {
  return maybeRow<PendingActionRow>(
    db.prepare(
      `SELECT *
       FROM pending_actions
...
export function getLatestOutboundMessage(db: SqlDatabase, remoteJid?: string): WhatsAppMessageRow | undefined {
  return maybeRow<WhatsAppMessageRow>(
    remoteJid
      ? db.prepare(
        `SELECT *
