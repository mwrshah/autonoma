...
function getHighestBranchNumber(repoPath: string): number {
	try {
		const output = execSync("git branch --list '[0-9]*'", { cwd: repoPath, timeout: 10_000, stdio: "pipe" })
			.toString()
			.trim();
		if (!output) return 0;
...
function resolveGtrWorktreePath(repoPath: string, branchName: string): string {
	try {
		const output = execSync(`git gtr go ${branchName}`, { cwd: repoPath, timeout: 5_000, stdio: "pipe" })
			.toString()
			.trim();
...
export function executeCreateWorktree(
	...
	if (!workstream) {
		return { ok: false, workstreamId, message: `Workstream ${workstreamId} not found`, usedGtr: false };
	}
	if (workstream.status !== "open") {
		return { ok: false, workstreamId, message: `Workstream ${workstreamId} is not open`, usedGtr: false };
	}
	...
	try {
		const worktrees = execSync("git worktree list --porcelain", { cwd: repoPath, timeout: 10_000, stdio: "pipe" })
			.toString();
				...
				if (lines[i] === `branch refs/heads/${resolvedBranch}`) {
					...
					for (let j = i; j >= 0; j--) {
						if (lines[j].startsWith("worktree ")) {
							...
							return {
								ok: true, workstreamId, worktreePath: existingPath, branchName: resolvedBranch,
	...
	try {
		...
		try {
			execSync(`git config "git-town-branch.${resolvedBranch}.parent" main`, { cwd: repoPath, timeout: 5_000, stdio: "pipe" });
		} catch {
